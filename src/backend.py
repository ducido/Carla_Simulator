...
class UNET():
    ...
    def build(self):
        ...
        inputs = Input(shape=(self.im_height, self.im_width, 3))
        conv1 = self.make_conv_block(64, inputs, 1)
        pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)

        conv2 = self.make_conv_block(128, pool1, 2)
        pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)

        conv3 = self.make_conv_block(256, pool2, 3)
        pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)
        ...
        conv5 = self.make_conv_block(256, up5, 5)

        up6 = Concatenate()([UpSampling2D(size=(2, 2))(conv5), conv2])
        conv6 = self.make_conv_block(128, up6, 6)

        up7 = Concatenate()([UpSampling2D(size=(2, 2))(conv6), conv1])
        conv7 = self.make_conv_block(64, up7, 7)
